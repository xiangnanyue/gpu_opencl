...
VideoCapture camera("./bourne.mp4");
    ...
    namedWindow(windowName); // Resizable window, might not work on Windows.
        ...
        int k_row = 3;
        int k_col = 3;
//		float g_kernel[9] = {0.95, 1.18, 0.95, 1.18, 1.48, 1.18, 0.95, 1.18, 0.95};
        float g_kernel[9] = {1,2,1, 2, 4, 2, 1,2,1};
        for (int i = 0; i<9; ++i) {
            g_kernel[i] = g_kernel[i]/16;
        }

        // create a sobel kernel in the horizontal direction
        float s_kernel[9] = {-1, 0, 1,-1, 0, 1, -1, 0, 1};
		float s_vert_kernel[9] = {-1, -1, -1, 0, 0, 0, 1, 1, 1};
        printf("the s_kernel is :\n");
        print_matrix(3, s_kernel);

		printf("the s_vert_kernel is :\n");
		print_matrix(3, s_vert_kernel);
...
float *input_c = s_vert_kernel;
float *input_d = s_kernel;

// create output for x-axis result and vertical for y-axis result
float *output = (float *) malloc(sizeof(float)*h*w);  // for the results returned from GPU
float *vertical = (float *) malloc(sizeof(float)*h*w);
float *horizontal = (float *) malloc(sizeof(float)*h*w);
...
cl_mem input_c_buf; // vertical
cl_mem input_d_buf; // horizontal
cl_mem output_buf; // num_devices elements
cl_mem vertical_buf; 
cl_mem horizontal_buf;
int status;
...
// for the GPU calculation
 ...
 // Input buffers. of type cl_mem
    ...
    input_d_buf = clCreateBuffer(context, CL_MEM_READ_ONLY,
        k_row*k_col*sizeof(float), NULL, &status);
	checkError(status, "Failed to create buffer for input B");	


    // Output buffer: horizontal_buf for x-direction edge, vertical_buf for y_direction
    output_buf = clCreateBuffer(context, CL_MEM_READ_WRITE,
        h*w*sizeof(float), NULL, &status); // change to one dimension
    checkError(status, "Failed to create buffer for output");
	
	
	vertical_buf = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
        h*w*sizeof(float), NULL, &status); // change to one dimension
    checkError(status, "Failed to create buffer for output");	
	

	horizontal_buf = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
        h*w*sizeof(float), NULL, &status); // change to one dimension
    checkError(status, "Failed to create buffer for output");
...
************************ */
    ...
    // Set kernel arguments.and enqueue for both g_kernel and s_kernel
//    unsigned argi = 0;
...
/******************
    the s_vert_kernel: sobel vertical edge detection 
************************ */
    ...
    status = clEnqueueWriteBuffer(queue, input_c_buf, CL_FALSE,
        0, k_row*k_col*sizeof(float), input_c, 0, NULL, &write_event[2]);
    checkError(status, "Failed to transfer input B");
	

	// set argument buff pointer for kernel
    status = clSetKernelArg(kernel, 0, sizeof(cl_mem), &dimension_buf);
    checkError(status, "Failed to set argument 0");
    ...
    status = clSetKernelArg(kernel, 2, sizeof(cl_mem), &input_c_buf);
    checkError(status, "Failed to set argument 2");

    status = clSetKernelArg(kernel, 3, sizeof(cl_mem), &vertical_buf);
    checkError(status, "Failed to set argument 3");
    ...
    // Read the result. This the final operation.
    status = clEnqueueReadBuffer(queue, vertical_buf, CL_TRUE,
        0, w*h*sizeof(float), vertical, 1, &kernel_event, &finish_event);


/******************
    the s_kernel: sobel horizontal edge detection 
************************ */
    // write buffer enqueue
    status = clEnqueueWriteBuffer(queue, dimension_buf, CL_FALSE,
        0, 2*sizeof(int), dimension, 0, NULL, &write_event[0]);
    ...
    status = clEnqueueWriteBuffer(queue, input_d_buf, CL_FALSE,
        0, k_row*k_col*sizeof(float), input_d, 0, NULL, &write_event[2]);
    checkError(status, "Failed to transfer input B");
    ...
    status = clSetKernelArg(kernel, 2, sizeof(cl_mem), &input_d_buf);
    checkError(status, "Failed to set argument 2");

    status = clSetKernelArg(kernel, 3, sizeof(cl_mem), &horizontal_buf);
    checkError(status, "Failed to set argument 3");
    ...
    // Read the result. This the final operation.
    status = clEnqueueReadBuffer(queue, horizontal_buf, CL_TRUE,
        0, w*h*sizeof(float), horizontal, 1, &kernel_event, &finish_event);

	
/************* 
end filtering once, record time 
**************/
   time (&end);
   ...
   tot += diff;
	...
	Mat verticalframe = Mat(h, w, CV_32FC1);
	Mat horizontalframe = Mat(h, w, CV_32FC1);
	
	memcpy(verticalframe.data, vertical, 4*h*w);
	memcpy(horizontalframe.data, horizontal, 4*h*w);

	verticalframe.convertTo(edge_x, CV_8U);    
	horizontalframe.convertTo(edge_y, CV_8U);
...
clReleaseMemObject(vertical_buf);
