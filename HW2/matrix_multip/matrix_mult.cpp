...
float *ref_output= (float *) malloc(sizeof(float)*N*N);  // for the results of CPU
float *output = (float *) malloc(sizeof(float)*N*N);  // for the results returned from GPU
	...
	for(unsigned i = 0; i < N; ++i) {
		for (unsigned j = 0; j < N; ++j) {
			...
			for (unsigned k = 0; k < N; ++k) {
				ref_output[idx] += input_a[i*N + k]*input_b[k*N +j];
			}
		}
	}
	time (&end);
	diff = difftime (end,start);
  	printf ("CPU took %f seconds to run.\n", diff );
...
// for the GPU calculation
     ...
     if (program == NULL)
	{
     ...
     kernel = clCreateKernel(program, "matrix_multip", NULL);
 ...
 // Input buffers. of type cl_mem
    ...
    output_buf = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
        N*N*sizeof(float), NULL, &status); // change to one dimension
	...
	status = clEnqueueWriteBuffer(queue, dimension_buf, CL_FALSE,
        0, sizeof(int), dimension, 0, NULL, &write_event[0]);
    checkError(status, "Failed to transfer dimension");	

    status = clEnqueueWriteBuffer(queue, input_a_buf, CL_FALSE,
        0, N*N*sizeof(float), input_a, 0, NULL, &write_event[1]);
    checkError(status, "Failed to transfer input A");

    status = clEnqueueWriteBuffer(queue, input_b_buf, CL_FALSE,
        0, N*N*sizeof(float), input_b, 0, NULL, &write_event[2]);
    checkError(status, "Failed to transfer input B");

    // Set kernel arguments.
    unsigned argi = 0;

	status = clSetKernelArg(kernel, argi++, sizeof(cl_mem), &dimension_buf);
...
// Verify results calculated by CPU and GPU
bool pass = true;
